...
class MatchTest extends \PHPUnit_Framework_TestCase
{
    ...
    function dataProviderForTestSimpleRead()
    {
        ...
        $word_range = array_merge(range("A", "Z"), range("a", "z"), $number_range, array(
            "_"
        ));
        return array(
            array(
            ...
            ), // the regex is actually '^[\\\]a\-]$' after PHP string parsing
            array(
                '^[\\n-\\r]$',
                array_map("chr", range(10, 13))
            ),
            array(
                '^\*$',
                array(
            ...
            array(
                '^[^a-z]$',
                array_diff($full_range, range("a", "z"))
            ),
            array(
                '^\x61$',
                array(
            ...
            array(
                '^\W$',
                array_diff($full_range, $word_range)
            ),
            array(
                '^\d$',
                $number_range
            ...
            array(
                '^\D$',
                array_diff($full_range, $number_range)
            ),
            array(
                '^\s$',
                array(
    ...
    function dataProviderForTestSimpleReadFail()
    {
        return array(
            array(
                ...
                "regex doesn't start with a BOS token"
            ),
            array(
                "$",
                \RuntimeException::class,
            ...
            array(
                "^",
                \RuntimeException::class,
                "regex doesn't end with a EOS token"
            ),
            array(
                "^a|b$",
                \RuntimeException::class,
