...
class Set extends Token
{
        ...
        foreach ($offset_array as $offset) {
            if ($offset - 1 !== $last_offset) {
                if (count($current_run) > 2) {
                    ...
                    $string .= Constant::escapeChar(chr($first_offset)) . "-" . Constant::escapeChar(chr($last_offset));
                } else {
                    $string .= implode("", array_map(array(
                        Constant::class,
                        "escapeChar"
                    ), array_map("chr", $current_run)));
                }
                $current_run = array();
            }
            $current_run[] = $offset;
    ...
    function charArray()
    {
        return $this->_is_positive ? $this->_char_array : array_diff(array_map("chr", range(0, 127)), $this->_char_array);
    }
